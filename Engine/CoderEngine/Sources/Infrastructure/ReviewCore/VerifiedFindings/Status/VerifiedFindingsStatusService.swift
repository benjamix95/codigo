...
public enum VerifiedFindingsProjectionBuilder {
    public static func build(
        ...
        if shouldPreferRustProjection, let bridged = buildWithRust(from: snapshot) {
            return bridged
        }
        return VerifiedFindingsProjectionLocalBuilder.build(from: snapshot)
    }
    ...
    private static var shouldPreferRustProjection: Bool {
        ProcessInfo.processInfo.environment["SOLOCODE_REVIEW_CORE_USE_RUST_PROJECTION"] == "1"
    }
}
