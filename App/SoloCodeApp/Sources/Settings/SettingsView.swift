...
struct SettingsView: View {
    ...
    func applyProviderSyncs<V: View>(_ content: V) -> some View {
        content
            .onChange(of: openaiApiKey) { _ in syncOpenAI() }
            .onChange(of: openaiModel) { _ in syncOpenAI() }
            .onChange(of: anthropicApiKey) { _ in syncAnthropic() }
            .onChange(of: anthropicModel) { _ in syncAnthropic() }
            .onChange(of: googleApiKey) { _ in syncGoogle() }
            .onChange(of: googleModel) { _ in syncGoogle() }
            .onChange(of: minimaxApiKey) { _ in syncMiniMax() }
            .onChange(of: minimaxModel) { _ in syncMiniMax() }
            .onChange(of: openrouterApiKey) { _ in syncOpenRouter() }
            .onChange(of: openrouterModel) { _ in syncOpenRouter() }
            .onChange(of: grokApiKey) { _ in syncGrok() }
            .onChange(of: grokModel) { _ in syncGrok() }
    }

    func applyCLISyncs<V: View>(_ content: V) -> some View {
        let codexCoreSyncs = content
            .onChange(of: codexPath) { _ in codexState.refresh(); syncCodex() }
            .onChange(of: codexSandbox) { _ in syncCodex(); saveCodexToml() }
            .onChange(of: codexAskForApproval) { _ in syncCodex() }
            .onChange(of: codexModelOverride) { _ in syncCodex(); saveCodexToml() }
            .onChange(of: codexReasoningEffort) { _ in syncCodex(); saveCodexToml() }
            .onChange(of: codexFastMode) { _ in syncCodex(); saveCodexToml() }
            .onChange(of: codexModelProvider) { _ in syncCodex(); saveCodexToml() }
        let codexSettingsSyncs = codexCoreSyncs
            .onChange(of: codexPreferResponsesWireAPI) { _ in syncCodex() }
            .onChange(of: codexSessionFullAccess) { _ in syncCodex() }
            .onChange(of: codexCustomGPT54Enabled) { _ in syncCodex(); saveCodexToml() }
            .onChange(of: codexNetworkAccess) { _ in saveCodexToml() }
            .onChange(of: codexAdditionalWriteRoots) { _ in saveCodexToml() }
            .onChange(of: codexCheckUpdate) { _ in saveCodexToml() }
            .onChange(of: codexDeveloperInstructions) { _ in saveCodexToml() }
            .onChange(of: codexVerbosity) { _ in saveCodexToml() }
        return codexSettingsSyncs
            .onChange(of: claudePath) { _ in syncClaude() }
    ...
    func applyBehaviorSyncs<V: View>(_ content: V) -> some View {
        content
            .onChange(of: globalYolo) { _ in syncCodex(); syncPlanProvider(); syncCodeReview() }
            .onChange(of: appUpdateCheckEnabled) { isEnabled in
                appUpdateCenter.setUpdateCheckEnabled(isEnabled)
            }
            .onChange(of: appUpdateManifestURL) { newURL in
                appUpdateCenter.setManifestURL(newURL)
            }
            .onChange(of: codebaseIndexEnabled) { _ in
                workspaceStore.indexActiveWorkspace()
                Task { await refreshIndexStatus() }
            }
            .onChange(of: codebaseIndexExcludedPaths) { _ in
                workspaceStore.indexActiveWorkspace()
            }
