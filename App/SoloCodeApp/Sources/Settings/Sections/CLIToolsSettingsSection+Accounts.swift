...
// MARK: - Multi-Account Provider Section
...
extension CLIToolsSettingsSection {
    ...
    func accountCard(_ account: CLIAccount, provider: CLIProviderKind) -> some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            accountStatusRow(account, isConnected: isConnected)
            accountActionsRow(account, isConnected: isConnected)
        }
        .padding(10)
    ...
    private func accountStatusRow(_ account: CLIAccount, isConnected: Bool) -> some View {
        HStack(spacing: 8) {
            settingsStatusBadge(connected: isConnected, label: accountStatusLabel(account))
            Spacer()
    ...
    @ViewBuilder
    private func accountActionsRow(_ account: CLIAccount, isConnected: Bool) -> some View {
        HStack(spacing: 6) {
            if !isConnected {
                Button("Sign In") {
                    loginSheetAccount = account
