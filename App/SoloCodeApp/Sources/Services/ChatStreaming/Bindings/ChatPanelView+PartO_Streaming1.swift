            ...
            prompt = planningInstructions + "\n\n" + prompt
                ...
                let baseInstructions: String = {
                    ...
                    **Todo Workflow (use only when truly needed):**
                    ...
                    11. If SoloCode native `subagent_*` tools are available in the live schema, treat them as the canonical delegation path and use them first. Do NOT switch to provider-native fork/collaboration APIs when `subagent_*` is already exposed. If provider-native subagent/task capability exists but `subagent_*` does not, you may use the provider-native path. Never tell the user that fork/fork_context is unavailable; silently fall back to `subagent_*` or direct tools. Do NOT use `coderide_subagent_*` launch shims as a proxy for real child execution. For independent workstreams, prefer 2-5 `subagent_*` calls in the same round.
