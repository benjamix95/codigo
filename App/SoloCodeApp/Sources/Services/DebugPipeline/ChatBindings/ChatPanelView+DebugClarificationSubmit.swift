    ...
    internal func submitDebugClarificationToAgent(_ submission: DebugClarificationSubmission) {
        ...
        guard let targetConversationId = conversationId else {
            appendTechnicalErrorMessage(
                "[Debug] Nessuna conversazione selezionata per inviare il chiarimento.",
                in: nil
            )
            return
        }
        ...
        let runtimeReviewPreference: Bool? = nil
        guard
        ...
        debugStore.isAwaitingUserClarification = false
        debugStore.clarificationQuestions = ""
        persistDebugState(for: targetConversationId)
        ...
        let turnId = UUID()
        let capturedAttachments: [ComposerAttachment] = []
        ...
        AgentDebugSessionNDJSONLog.append(
            hypothesisId: "SEND",
            location: "submitDebugClarificationToAgent",
            message: "debug_panel_clarification_send",
            data: [
