...
/// Filter activities by conversation ID.
...
// MARK: - Worker Batch Selection

/// Return the latest contiguous batch of review-worker-plan activities.
func latestReviewWorkerPlanBatch(
    in activities: [TaskActivity]
) -> [TaskActivity] {
    guard let lastPlanIndex = activities.lastIndex(
        where: { $0.type == "review-worker-plan" }
    ) else {
        return []
    }
    ...
    while firstPlanIndex > activities.startIndex {
        ...
        guard activities[previousIndex].type == "review-worker-plan"
        else { break }
...
/// Select worker plan activities from review activities.
/// Prefers plans after last round boundary, falls back to latest batch.
func selectReviewWorkerActivities(
    from activities: [TaskActivity]
) -> [TaskActivity] {
    guard !activities.isEmpty else { return [] }

    if let boundary = activities.lastIndex(
        where: { $0.type == "review-fix-round" }
    ) {
...
/// Sort worker activities for display using natural worker ID ordering.
func sortedReviewWorkerPlanActivitiesForDisplay(
    _ activities: [TaskActivity]
) -> [TaskActivity] {
    activities.sorted { lhs, rhs in
        ...
        if lhsHasWorker && rhsHasWorker && lhsWorkerID != rhsWorkerID {
            return lhsWorkerID.localizedStandardCompare(rhsWorkerID)
                == .orderedAscending
        }
        if lhsHasWorker != rhsHasWorker { return lhsHasWorker }
...
// MARK: - Swarm Card Helpers

/// Check if a swarm card belongs to code review.
func isCodeReviewSwarmCard(_ card: SwarmLiveCardState) -> Bool {
    if card.swarmId.hasPrefix("review-") { return true }
...
/// Check if a swarm card belongs to a specific conversation.
func reviewCardBelongsToConversation(
    ...
    return !scopedTaskActivitiesForConversation(
        card.recentEvents, conversationId: conversationId
    ).isEmpty
}

/// Check if a swarm card belongs to a specific session.
