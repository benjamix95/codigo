...
struct DebugPanelView: View {
    @EnvironmentObject internal var pipelineIntegrationService: PipelineIntegrationService
...
extension DebugPanelView {
    @ViewBuilder
    var suppressedDebugProjectionBufferBanner: some View {
        let _ = pipelineIntegrationService.debugProjectionBufferRevision
        if let cid = conversationId,
           pipelineIntegrationService.isDebugProjectionSuppressed(for: cid) {
            let pending = pipelineIntegrationService.pendingBufferedDebugEventCount(for: cid)
            if pending > 0 {
