(function() {
  ...
  function provideRequest(editor) {
    ...
    return window.CodigoMonacoBridge.requestContext(editor, state.currentPane, state.currentPath);
  }
  ...
  function register(monacoInstance) {
    ...
    monacoInstance.languages.registerHoverProvider('swift', {
      provideHover: async function(model, position) {
        const request = provideRequest(window.CodigoMonacoMain.editor());
        ...
        const payload = await window.CodigoMonacoBridge.request('hoverRequested', request);
        if (!payload || !payload.contents) return null;
        return {
          range: window.CodigoMonacoLang.wordRange(model, position, request.word),
    ...
    monacoInstance.languages.registerDefinitionProvider('swift', {
      provideDefinition: async function(model, position) {
        ...
        const payload = await window.CodigoMonacoBridge.request('definitionRequested', request);
        if (!payload || !payload.locations) return [];
        return payload.locations.map(function(location) {
          return {
            uri: window.CodigoMonacoLang.uriForPath(location.filePath, request.pane),
    ...
    monacoInstance.languages.registerReferenceProvider('swift', {
      provideReferences: async function(model, position) {
        ...
        request.word = (model.getWordAtPosition(position) || {}).word || '';
        if (!request.word) return [];
        const payload = await window.CodigoMonacoBridge.request('referencesRequested', request);
        if (!payload || !payload.locations) return [];
        return payload.locations.map(function(location) {
          return {
            uri: window.CodigoMonacoLang.uriForPath(location.filePath, request.pane),
    ...
    monacoInstance.languages.registerRenameProvider('swift', {
      provideRenameEdits: async function(model, position, newName) {
        ...
        const payload = await window.CodigoMonacoBridge.request('renameRequested', request);
        if (!payload || !payload.edits) return { edits: [] };
        return {
          edits: payload.edits.map(function(edit) {
            return {
              resource: window.CodigoMonacoLang.uriForPath(edit.filePath, request.pane),
              edit: {
    ...
    monacoInstance.languages.registerDocumentSymbolProvider('swift', {
      provideDocumentSymbols: async function(model) {
        const request = provideRequest(window.CodigoMonacoMain.editor());
        const payload = await window.CodigoMonacoBridge.request('outlineRequested', request);
  ...
  window.CodigoMonacoProviders = { register: register };
})();
