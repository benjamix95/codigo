...
extension UnifiedToolRuntime {
    ...
    static func syntheticMCPMetadata(
        ...
        if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) {
    ...
    static func mapPlanLifecycleMCPEvent(
        ...
        case "plan_create":
            ...
            if let chosenPath = firstNonEmptyString(in: arguments, keys: ["chosen_path"]) { payload["chosen_path"] = chosenPath }
            if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) { payload["conversation_id"] = conversationID }
        ...
        case "plan_read":
            var payload: [String: String] = [:]
            if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) { payload["conversation_id"] = conversationID }
            if let includeHistory = firstNonEmptyString(in: arguments, keys: ["include_history"]) { payload["include_history"] = includeHistory }
            if let historyLimit = firstNonEmptyString(in: arguments, keys: ["history_limit"]) { payload["history_limit"] = historyLimit }
        ...
        case "plan_step_reorder":
            var payload: [String: String] = [:]
            if let ordered = jsonStringArgument(in: arguments, keys: ["ordered_step_ids"]) { payload["ordered_step_ids"] = ordered }
            if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) { payload["conversation_id"] = conversationID }
            return payload.isEmpty ? nil : ("plan_step_reorder", payload)

        case "plan_step_dependency_set":
            var payload: [String: String] = [:]
            if let stepID = firstNonEmptyString(in: arguments, keys: ["step_id"]) { payload["step_id"] = stepID }
            if let dependsOn = jsonStringArgument(in: arguments, keys: ["depends_on"]) { payload["depends_on"] = dependsOn }
            if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) { payload["conversation_id"] = conversationID }
        ...
        case "plan_diff":
            var payload: [String: String] = [:]
            if let fromSnapshotID = firstNonEmptyString(in: arguments, keys: ["from_snapshot_id"]) { payload["from_snapshot_id"] = fromSnapshotID }
            if let toSnapshotID = firstNonEmptyString(in: arguments, keys: ["to_snapshot_id"]) { payload["to_snapshot_id"] = toSnapshotID }
            if let conversationID = firstNonEmptyString(in: arguments, keys: ["conversation_id"]) { payload["conversation_id"] = conversationID }
