...
public final class MultiSwarmReviewProvider: LLMProvider, @unchecked Sendable {
        ...
        await withTaskGroup(of: (String, String).self) { group in
            for p in partitions {
                group.addTask {
                    ...
                    yieldRaw(
                        .raw(
                            type: "agent", payload: ["title": "Swarm \(p.id)", "detail": "started"])
                    )
                    yieldRaw(
                        .raw(
                    ...
                    do {
                        let stream = try await analysisProvider.send(
                            prompt: reviewPrompt, context: scoped, imageURLs: nil)
                    ...
                    if output.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        output =
                            "\(missingWorkerOutputPrefix)\(p.id). Verifica autenticazione provider o timeout.]"
                    }
                    yieldRaw(
                        .raw(
                    ...
                    yieldRaw(
                        .raw(
                            type: "agent",
                            payload: ["title": "Swarm \(p.id)", "detail": "completed"]))
                    return (p.id, output)
                }
            }
    ...
    private func hasSignificantFindings(_ reports: [(partitionId: String, output: String)]) -> Bool
    {
        ...
        let triggers = [
            "priorità alta", "priorità alta:", "bug", "correggere", "problema", "errore", "warning",
        ...
        async throws -> AsyncThrowingStream<StreamEvent, Error>
    {
        ...
        if config.analysisBackend == "claude", claudeProvider != nil {
            analysisProvider = ClaudeCLIProvider(
                allowedTools: analysisClaudeTools, executionController: nil, executionScope: .review
            )
        ...
        return AsyncThrowingStream { continuation in
            Task {
                ...
                guard analysisProvider.isAuthenticated() else {
                    continuation.yield(
                        .textDelta(
                            "**Analisi non avviata:** provider di analisi non autenticato. Verifica Codex CLI/Claude CLI nelle impostazioni.\n"
                        ))
                ...
                let useUncommitted =
                    promptLower.contains("non committati") || promptLower.contains("uncommitted")
                    || promptLower.contains("non committate") || promptLower.contains("modificati")
                    || promptLower.contains("changed") || promptLower.contains("modified")
                    || promptLower.contains("staged")
                ...
                if config.enabledPhases == .analysisAndExecution {
                    continuation.yield(
                        .textDelta("- Provider esecuzione Fase 2: `\(execBackendLabel)`\n"))
                }
                continuation.yield(
                    .textDelta("- Scope: `\(scope == .uncommitted ? "uncommitted" : "all")`\n"))
                ...
                if scope == .uncommitted {
                    ...
                    if !preview.isEmpty {
                        continuation.yield(
                            .textDelta(
                                "- Esempio file scope: `\(preview.joined(separator: "`, `"))`\n"))
                    }
                }
                continuation.yield(.textDelta("\n"))
                ...
                let runPhase2 =
                    ...
                    && (config.yoloMode || prompt.lowercased().contains("procedi")
                        || prompt.lowercased().contains("applica")
                        || prompt.lowercased().contains("sì"))
                ...
                for round in 1...config.maxReviewRounds {
                    if round > 1 {
                        ...
                        continuation.yield(
                            .textDelta(
                                "Partizioni: \(partitions.count) (totale \(partitions.flatMap(\.paths).count) file)\n\n"
                            ))
                        continuation.yield(
                            .textDelta(
                                "Avvio analisi in parallelo su \(partitions.count) swarm...\n\n"))
                        reports = await runPhase1AnalysisStreaming(
                            partitions: partitions,
                            context: context,
                            analysisProvider: analysisProvider,
                    ...
                    if allFailed && !reports.isEmpty {
                        ...
                        let ordered: [ReviewFailureReason] = [
                            .auth, .cliExit, .timeout, .emptyOutput, .unknown,
                        ]
                    ...
                    let steps = await coordinator.planExecution(
                        swarmFileClaims: partitions.map { ($0.id, Set($0.paths)) })
                    ...
                    for step in steps {
                        guard let partition = partitions.first(where: { $0.id == step.swarmId })
                        else { continue }
                        ...
                        let execPrompt =
                            "\(reviewExecutionPrompt)\n\nFile nella partizione: \(partition.paths.joined(separator: ", "))"
                        do {
                            let stream = try await execProvider.send(
                                prompt: execPrompt, context: scoped, imageURLs: nil)
                            for try await event in stream {
