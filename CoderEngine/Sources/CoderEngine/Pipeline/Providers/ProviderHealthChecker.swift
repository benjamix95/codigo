...
public actor ProviderHealthChecker {
    ...
    public func probeProvider(_ providerId: String) async {
        guard providerStates[providerId] != nil else { return }
    ...
    /// Registra l'esito di una esecuzione reale senza effettuare una probe separata.
    /// Questo permette di rendere health-aware anche il routing runtime dei provider.
    public func recordExecutionResult(
        ...
        state.totalProbes += 1
        state.lastProbeTime = timestamp
        state.lastProbeSuccess = success

        appendToErrorWindow(&state, success: success)
