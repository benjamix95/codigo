...
impl Backend {
    pub(crate) fn list_resources(&mut self, payload: Value) -> Result<Value, BackendError> {
        ...
        let resources =
            self.with_process_for_server(&server_id, "list_resources", |process, config| {
    ...
    pub(crate) fn read_resource(&mut self, payload: Value) -> Result<Value, BackendError> {
        ...
        let server_id =
            self.resolve_server_identity(request.server_id, request.server_name, request.server)?;
        let contents =
            self.with_process_for_server(&server_id, "read_resource", |process, _| {
    ...
    pub(crate) fn list_resource_templates(
        ...
        let server_id =
            self.resolve_server_identity(request.server_id, request.server_name, request.server)?;
        let templates = self.with_process_for_server(
            &server_id,
            "list_resource_templates",
            |process, config| process.list_resource_templates(&config.id, &config.name),
        )?;
    ...
    pub(crate) fn list_prompts(&mut self, payload: Value) -> Result<Value, BackendError> {
        ...
        let server_id =
            self.resolve_server_identity(request.server_id, request.server_name, request.server)?;
        let prompts =
            self.with_process_for_server(&server_id, "list_prompts", |process, config| {
