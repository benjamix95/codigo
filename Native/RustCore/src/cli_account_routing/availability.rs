...
pub(crate) fn available_accounts(
    ...
    items.retain(|account| account.health.cooldown_until.map(|value| value <= now()).unwrap_or(true));
    items.retain(|account| usage.get(&account.id).map(|totals| !exceeds_policy(account, totals)).unwrap_or(true));
    items
}
...
pub(crate) fn exceeds_policy(
    ...
    account.quota.daily_limit_usd.map(|value| usage.day_cost >= value).unwrap_or(false)
        || account.quota.weekly_limit_usd.map(|value| usage.week_cost >= value).unwrap_or(false)
        || account.quota.monthly_limit_usd.map(|value| usage.month_cost >= value).unwrap_or(false)
        || account.quota.daily_token_limit.map(|value| usage.day_tokens >= value).unwrap_or(false)
        || account.quota.weekly_token_limit.map(|value| usage.week_tokens >= value).unwrap_or(false)
        || account.quota.monthly_token_limit.map(|value| usage.month_tokens >= value).unwrap_or(false)
}

pub(crate) fn usage_by_account(
    items: &[CLIAccountUsageTotalsSnapshot],
) -> BTreeMap<String, CLIAccountUsageTotalsSnapshot> {
    items.iter().map(|item| (item.account_id.clone(), item.clone())).collect()
}
