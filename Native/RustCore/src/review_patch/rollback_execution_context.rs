...
pub fn build_rollback_execution_context(
    ...
    ReviewPatchRollbackExecutionContextResponse::success(format!(
        "{}-rollback",
        request.patch_id
    ))
}
...
mod tests {
    ...
    fn build_rollback_execution_context_requires_applied_status_and_ref() {
        let response = build_rollback_execution_context(ReviewPatchRollbackExecutionContextRequest {
            schema_version: 1,
    ...
    fn build_rollback_execution_context_derives_patch_prefix() {
        ...
        assert_eq!(response.patch_file_prefix.as_deref(), Some("patch-1-rollback"));
    }
}
