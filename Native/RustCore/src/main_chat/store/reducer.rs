...
pub fn handle_action(request: MainChatStoreActionRequest) -> MainChatStoreResponse {
    ...
    match request.action.as_str() {
        "create_conversation" => {
            conversations::create_conversation(request.snapshot.clone(), &request)
        }
        "delete_conversation" => {
        ...
        "insert_message_before" => {
            messages::insert_message_before(request.snapshot.clone(), &request)
        }
        ...
        "sync_assistant_content" => {
            messages::sync_assistant_content(request.snapshot.clone(), &request)
        }
        ...
        "attach_plan_to_message" => {
            plans::attach_plan_to_message(request.snapshot.clone(), &request)
        }
        ...
        "rewind_to_checkpoint" => {
            ...
            checkpoints::rewind_to_checkpoint(
        ...
        "rewind_to_message_count" => {
            ...
            let Some(message_count) = request.message_count else {
                return MainChatStoreResponse::error(
                    ...
                    "messageCount is required",
                );
            };
            checkpoints::rewind_to_message_count(
                request.snapshot.clone(),
                message_count,
                conversation_id,
            )
        ...
        | "set_context_memory_summary" => {
            conversations::update_conversation_preferences(request.snapshot.clone(), &request)
        }
