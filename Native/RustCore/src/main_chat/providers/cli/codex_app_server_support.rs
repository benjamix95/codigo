...
pub(super) fn send_request(stdin: &mut ChildStdin, id: i64, method: &str, params: Value) -> Result<(), String> {
...
pub(super) fn send_notification(stdin: &mut ChildStdin, method: &str, params: Value) -> Result<(), String> {
    write_json_line(stdin, &json!({"jsonrpc": "2.0", "method": method, "params": params}))
}

pub(super) fn send_result(stdin: &mut ChildStdin, id: JsonRpcId, result: Value) -> Result<(), String> {
    write_json_line(stdin, &JsonRpcResponse::ok(id, result))
}

pub(super) fn send_error(stdin: &mut ChildStdin, id: JsonRpcId, code: i64, message: String) -> Result<(), String> {
...
pub(super) fn write_json_line<T: serde::Serialize>(stdin: &mut ChildStdin, payload: &T) -> Result<(), String> {
    let encoded = serde_json::to_string(payload).map_err(|error| error.to_string())?;
    stdin.write_all(encoded.as_bytes()).and_then(|_| stdin.write_all(b"\n")).and_then(|_| stdin.flush()).map_err(|error| format!("codex_app_server_write_failed:{error}"))
}

pub(super) fn json_rpc_id(value: &Value) -> Result<JsonRpcId, String> {
    match value.get("id") {
        Some(Value::Number(number)) => number.as_i64().map(JsonRpcId::Number).ok_or_else(|| "invalid_jsonrpc_id".to_string()),
