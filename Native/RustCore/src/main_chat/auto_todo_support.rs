...
pub fn preferred_title(current_title: &str, candidate_title: &str) -> String {
    ...
    if current.is_empty() || (current == AUTO_TODO_GENERIC_FALLBACK_TITLE && candidate != AUTO_TODO_GENERIC_FALLBACK_TITLE) {
        return candidate.to_string();
    }
    current.to_string()
}

pub fn preferred_active_form(current_active_form: &str, immediate_label: String, title: &str) -> String {
...
pub fn auto_todo_title(request: &MainChatUiIntentRequest) -> String {
    ...
    if let Some(path) = first_non_empty(&[request.payload.get("path"), request.payload.get("file")]) {
        return format!("Complete changes on {}", path.rsplit('/').next().unwrap_or(path));
    }
...
pub fn linked_files_from_request(request: &MainChatUiIntentRequest) -> Vec<String> {
    ...
    for key in ["path", "file", "files"] {
        if let Some(raw) = request.payload.get(key) {
            for item in raw.split(',').map(|value| value.trim()).filter(|value| !value.is_empty()) {
...
pub fn payload(request: &MainChatUiIntentRequest, key: &str) -> String {
    request.payload.get(key).map(|value| value.trim().to_string()).unwrap_or_default()
}
...
fn is_placeholder_title(raw_title: &str) -> bool {
    ...
    matches!(
        normalized.as_str(),
        "task" | "tasks" | "todo" | "todos" | "step" | "steps" | "analysis" | "workflow" | "execution" | "plan"
...
fn first_non_empty<'a>(values: &[Option<&'a String>]) -> Option<&'a str> {
    values.iter().flatten().map(|value| value.trim()).find(|value| !value.is_empty())
}
