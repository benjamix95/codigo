use super::helpers::{
    array, array_mut, build_outcome, event, is_open_status, now_reference_seconds,
    scope_description,
};
use super::models::{
    ReviewRegistryActionRequest, ReviewSessionActionRequest, ReviewSessionResponse,
};
...
pub fn apply_registry_action(request: ReviewRegistryActionRequest) -> ReviewSessionResponse {
    ...
    if mutation.is_error {
        return ReviewSessionResponse::error(
            "registry_mutation_failed",
            mutation
                .message
                .as_deref()
                .unwrap_or("registry mutation failed"),
        );
    }
...
pub fn apply_action(request: ReviewSessionActionRequest) -> ReviewSessionResponse {
    ...
    let result = match request.operation.as_str() {
        ...
        "mark_analysis_started" => mark_simple(
            &mut snapshot,
            "analyzing",
            "analysis",
            "analysis_started",
        ...
        "mark_audit_started" => {
            mark_audit_started(&mut snapshot, request.tool_name.as_deref(), now)
        }
        ...
        "set_active_worker_count" => set_number(
            &mut snapshot,
            "activeWorkerCount",
            request.count.unwrap_or(0),
        ),
        "mark_worker_spawned" => mark_worker(
            &mut snapshot,
            "worker_spawned",
            request.worker_id,
            request.title,
        ...
        "mark_worker_completed" => mark_worker(
            &mut snapshot,
            "worker_completed",
            request.worker_id,
            request.title,
        ...
        "mark_round_completed" => {
            mark_round_completed(&mut snapshot, request.round.unwrap_or(0), now)
        }
        "mark_testing_started" => mark_simple(&mut snapshot, "testing", "testing", "", "", now),
        "mark_test_result" => mark_test_result(
            &mut snapshot,
            request.test_status.as_deref(),
            request.result_detail.as_deref(),
            now,
        ),
        "mark_re_review_started" => {
            mark_re_review_started(&mut snapshot, request.round.unwrap_or(0), now)
        }
        ...
        "add_candidate" => {
            add_candidates(&mut snapshot, request.candidate.into_iter().collect(), now)
        }
        "add_candidates" => {
            add_candidates(&mut snapshot, request.candidates.unwrap_or_default(), now)
        }
        "update_candidate_status" => update_candidate_status(&mut snapshot, &request, now),
        "promote_candidate_to_finding" => {
            promote_candidate_to_finding(&mut snapshot, request.candidate_id.as_deref(), now)
        }
        "replace_open_findings" => replace_open_findings(
            &mut snapshot,
            request.files,
            request.findings.unwrap_or_default(),
            now,
        ),
        "mark_patch_prepare_failed" => mark_patch_prepare_failed(
            &mut snapshot,
        ...
        _ => Err(format!(
            "Unsupported review session operation: {}",
            request.operation
        )),
    };
...
fn start(
    ...
    snapshot["events"] = json!([event(
        "session_started",
        Some(format!(
            "Review started with scope: {}",
            scope_description(snapshot.get("scope").unwrap_or(&Value::Null))
        )),
        json!({"scope": scope_description(snapshot.get("scope").unwrap_or(&Value::Null)), "file_count": snapshot.get("scope").and_then(|v| v.get("files")).and_then(Value::as_array).map(|a| a.len()).unwrap_or(0).to_string()}),
        now
    )]);
...
fn complete(snapshot: &mut Value, now: f64) -> Result<(), String> {
    ...
    array_mut(snapshot, "events")?.push(event(
        "session_completed",
        Some(format!("Review completed with {} findings", findings_count)),
        json!({}),
        now,
    ));
    Ok(())
...
fn record_audit_result(
    ...
    if let Some(audit) = snapshot.get_mut("audit").and_then(Value::as_object_mut) {
        audit
            .entry("toolCoverage".to_string())
            .or_insert_with(|| json!({}))[tool_name] = json!(coverage);
        audit
            .entry("toolDurationsMs".to_string())
            .or_insert_with(|| json!({}))[tool_name] = json!(duration_ms);
        audit
            .entry("toolFindingsCounts".to_string())
            .or_insert_with(|| json!({}))[tool_name] = json!(findings_count);
        audit
            .entry("toolAdapters".to_string())
            .or_insert_with(|| json!({}))[tool_name] = Value::Array(adapters_used);
    }
...
fn start_round(snapshot: &mut Value, round: i64, now: f64) -> Result<(), String> {
    ...
    let max_rounds = snapshot
        .get("config")
        .and_then(|v| v.get("maxRounds"))
        .and_then(Value::as_i64)
        .unwrap_or(3);
    array_mut(snapshot, "events")?.push(event(
        "round_started",
        Some(format!("Round {round}/{max_rounds}")),
        json!({"round": round.to_string(), "max_rounds": max_rounds.to_string()}),
        now,
    ));
    Ok(())
}
...
fn mark_worker(
    ...
    array_mut(snapshot, "events")?.push(event(
        kind,
        title,
        json!({"worker_id": worker_id.unwrap_or_default()}),
        now,
    ));
    Ok(())
}
...
fn mark_round_completed(snapshot: &mut Value, round: i64, now: f64) -> Result<(), String> {
    array_mut(snapshot, "events")?.push(event(
        "round_completed",
        Some(format!("Round {round} completed")),
...
fn add_findings(snapshot: &mut Value, new_findings: Vec<Value>, now: f64) -> Result<(), String> {
    for finding in new_findings {
        ...
        array_mut(snapshot, "events")?.push(event(
            "finding_added",
            Some(format!("[{severity}] {file_path}")),
            json!({"finding_id": id, "severity": severity, "file_path": file_path}),
            now,
        ));
    }
    Ok(())
...
fn add_candidates(
    ...
    for candidate in new_candidates {
        ...
        array_mut(snapshot, "events")?.push(event(
            ...
            json!({"candidate_id": id, "file_path": file_path}),
            now,
        ));
    }
    Ok(())
...
fn update_candidate_status(
    ...
    if let Some(candidates) = snapshot.get_mut("candidates").and_then(Value::as_array_mut) {
        ...
        candidate["verificationMethod"] = request
            .method
            .clone()
            .map(Value::String)
            .unwrap_or(Value::Null);
        candidate["verificationReport"] = request
            .report
            .clone()
            .map(Value::String)
            .unwrap_or(Value::Null);
        candidate["falsePositiveReason"] = request
            .false_positive_reason
            .clone()
            .map(Value::String)
            .unwrap_or(Value::Null);
        candidate["verifiedAt"] = if status == "verified" {
            json!(now)
        } else {
            Value::Null
        };
        event_value = match status {
            "verified" => Some(event(
                "candidate_verified",
                Some(format!("Candidate {candidate_id} verified")),
                json!({"candidate_id": candidate_id}),
                now,
            )),
            "rejected_false_positive" => Some(event(
                "candidate_rejected",
                Some(format!("Candidate {candidate_id} rejected")),
                json!({"candidate_id": candidate_id, "reason": request.false_positive_reason.clone().unwrap_or_else(|| "false_positive".to_string())}),
                now,
            )),
            _ => None,
        };
...
fn promote_candidate_to_finding(
    snapshot: &mut Value,
    ...
    if array(snapshot, "findings")?
        .iter()
        .any(|item| item.get("id").and_then(Value::as_str) == Some(candidate_id))
    {
        return Ok(());
    }
    let finding = json!({
...
fn replace_open_findings(
    snapshot: &mut Value,
    files: Option<Vec<String>>,
    new_findings: Vec<Value>,
    now: f64,
) -> Result<(), String> {
...
fn mark_open_findings(snapshot: &mut Value, files: Option<Vec<String>>) -> Result<(), String> {
    ...
    if let Some(findings) = snapshot.get_mut("findings").and_then(Value::as_array_mut) {
        for finding in findings.iter_mut() {
            ...
            let status = finding
                ...
                .and_then(Value::as_str)
                .unwrap_or("open");
            let should_mark = files
                .as_ref()
                .map(|list| list.iter().any(|item| item == file))
                .unwrap_or(true);
            if should_mark && is_open_status(status) {
                let next = if files.is_some() {
                    "patch_applied"
                } else {
                    "fix_applied"
                };
    ...
    for finding_id in changed_ids {
        array_mut(snapshot, "events")?.push(event(
            "finding_fix_applied",
            Some(format!("Fix applied for finding {finding_id}")),
...
fn mark_patch_prepare_failed(
    ...
    snapshot["outcome"] = build_outcome(
        snapshot,
        Some(format!(
            "Patch preparation failed for {finding_id}: {message}"
        )),
    );
    Ok(())
}
...
fn command_payload(request: &ReviewSessionActionRequest) -> HashMap<String, String> {
    ...
    if let Some(finding_id) = &request.finding_id {
        payload.insert("finding_id".to_string(), finding_id.clone());
    }
    ...
    if let Some(comment) = &request.comment {
        if let Some(author) = comment.get("author").and_then(Value::as_str) {
            payload.insert("author".to_string(), author.to_string());
        }
        ...
        for (lhs, rhs) in [
            ...
            if let Some(value) = config.get(rhs) {
                ...
                if let Some(text) = text {
                    payload.insert(lhs.to_string(), text);
                }
            }
        }
    }
...
fn mutation_configured_snapshot(
    mutation: crate::review_command::models::ReviewCommandMutationResponse,
    mut snapshot: Value,
) -> Value {
    if let Some(findings) = mutation.findings {
        snapshot["findings"] = Value::Array(findings);
    }
    if let Some(patches) = mutation.patches {
        snapshot["patches"] = Value::Array(patches);
    }
    if let Some(events) = mutation.events {
        snapshot["events"] = Value::Array(events);
    }
    if let Some(config) = mutation.config {
