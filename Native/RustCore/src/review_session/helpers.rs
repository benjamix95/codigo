...
pub fn object_mut<'a>(value: &'a mut Value, key: &str) -> Result<&'a mut Map<String, Value>, String> {
    value.get_mut(key)
...
pub fn scope_description(scope: &Value) -> String {
    let file_count = scope
        ...
        .map(|items| items.len())
        .unwrap_or(0);
    match scope.get("type").and_then(Value::as_str).unwrap_or("uncommitted") {
        ...
        "against_ref" => format!(
            "vs {} ({file_count} files)",
            scope.get("ref").and_then(Value::as_str).unwrap_or("unknown")
        ),
...
pub fn event(kind: &str, detail: Option<String>, metadata: Value, timestamp: f64) -> Value {
    ...
    Value::Object(Map::from_iter([
        ("id".to_string(), Value::String(format!("rust-event-{kind}-{timestamp:.6}"))),
        ("type".to_string(), Value::String(kind.to_string())),
        ("timestamp".to_string(), json!(timestamp)),
        ("detail".to_string(), detail.map(Value::String).unwrap_or(Value::Null)),
        ("metadata".to_string(), Value::Object(metadata)),
    ]))
}

pub fn build_outcome(snapshot: &Value, summary_override: Option<String>) -> Value {
    let findings = snapshot.get("findings").and_then(Value::as_array).cloned().unwrap_or_default();
    let candidates = snapshot.get("candidates").and_then(Value::as_array).cloned().unwrap_or_default();
    let patches = snapshot.get("patches").and_then(Value::as_array).cloned().unwrap_or_default();
    ...
    let patches_ready = patches
        .iter()
        .filter(|item| matches!(item.get("status").and_then(Value::as_str), Some("verified" | "draft")))
        .count();
    let patches_applied = patches.iter().filter(|item| item.get("status").and_then(Value::as_str) == Some("applied")).count();
    let prs_opened = patches
        .iter()
        .filter(|item| matches!(item.get("prStatus").and_then(Value::as_str), Some("opened")) || item.get("status").and_then(Value::as_str) == Some("pr_opened"))
        .count();
    let merged_patches = patches
        .iter()
        .filter(|item| matches!(item.get("mergeStatus").and_then(Value::as_str), Some("merged")) || item.get("status").and_then(Value::as_str) == Some("merged"))
        .count();
    let conflicts_detected = patches
        .iter()
        .map(|item| item.get("conflicts").and_then(Value::as_array).map(|items| items.len()).unwrap_or(0))
        .sum::<usize>();
    let manual_action_required = patches.iter().any(|item| {
        matches!(item.get("status").and_then(Value::as_str), Some("conflict" | "apply_failed"))
            || item.get("mergeStatus").and_then(Value::as_str) == Some("blocked")
    }) || candidates.iter().any(|item| item.get("verificationStatus").and_then(Value::as_str) == Some("inconclusive"));
    let tests_status = snapshot.get("lastTestStatus").cloned().unwrap_or(Value::Null);
    json!({
