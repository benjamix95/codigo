...
pub fn handle_review_tool(request: ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    match request.tool_name.as_str() {
        ...
        "review_apply_fix" | "review_dismiss" | "review_comment" | "review_configure"
        | "review_verify_finding" | "review_prepare_patch" | "review_apply_patch"
        | "review_verify_patch" | "review_revalidate_finding" | "review_rollback_patch"
        | "review_close_finding" | "review_open_pr" | "review_merge_pr" | "review_resolve_conflicts" => {
            queueable_review_action(&request)
        }
...
fn review_start(request: &ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    ...
    if !matches!(scope.as_str(), "uncommitted" | "staged" | "against_ref") {
        return ReviewMCPToolResponse::err(format!("Error: invalid scope '{}'. Use: uncommitted, staged, against_ref", scope));
    }
    if scope == "against_ref" && trimmed_arg(&request.args, "ref").is_empty() {
        return ReviewMCPToolResponse::err("Error: 'ref' parameter is required when scope=against_ref");
    }
    if let Some(session_id) = request.args.get("session_id").cloned().or_else(|| request.args.get("sessionId").cloned()) {
        if !valid_session_id(&session_id) {
            return ReviewMCPToolResponse::err("Error: invalid session_id. Use only letters, numbers, hyphen, or underscore");
        }
    }
    if let Some(value) = request.args.get("analysis_only") {
        let normalized = value.trim().to_lowercase();
        if !normalized.is_empty() && !matches!(normalized.as_str(), "1" | "0" | "true" | "false" | "yes" | "no" | "y" | "n") {
            return ReviewMCPToolResponse::err("Error: analysis_only must be a boolean value");
        }
    }
    if let Some(value) = request.args.get("max_workers").filter(|value| !value.trim().is_empty()) {
        match value.trim().parse::<i32>() {
            Ok(value) if (1..=12).contains(&value) => {}
            _ => return ReviewMCPToolResponse::err("Error: max_workers must be 1-12"),
        }
    }
    if let Some(value) = request.args.get("max_rounds").filter(|value| !value.trim().is_empty()) {
        match value.trim().parse::<i32>() {
...
fn review_findings(request: &ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    ...
    let lines = request
        ...
        .collect::<Vec<_>>();
    ReviewMCPToolResponse::ok(format!("Findings ({}):\n{}", request.review_findings_payload.len(), lines.join("\n")))
}
...
fn review_list_sessions(request: &ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    ...
    let lines = request.review_snapshots.iter().map(|snapshot| {
        format!(
            "{} | phase={} | stage={} | scope={} | findings={}",
            snapshot.session_id,
            snapshot.phase,
            snapshot.stage,
            snapshot.scope_type.clone().unwrap_or_else(|| "unknown".to_string()),
...
fn review_get_outcome(request: &ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    ...
    let lines = payload_line_map(
        payload,
        &[
            "summary", "verified_findings", "false_positives", "patches_ready", "patches_applied",
            "prs_opened", "merged_patches", "conflicts_detected", "manual_action_required",
            "tests_status",
        ],
    );
    ReviewMCPToolResponse::ok(lines.join("\n"))
...
fn queueable_review_action(request: &ReviewMCPToolRequest) -> ReviewMCPToolResponse {
    ...
    let Some(snapshot) = request.active_review_snapshot.as_ref() else {
        return ReviewMCPToolResponse::err(format!("Error: session_id '{}' was not found", session_id));
    };
    ...
    {
        return ReviewMCPToolResponse::err(format!("Error: finding_id '{}' does not belong to session_id '{}'", finding_id, session_id));
    }
    if request.tool_name == "review_apply_patch" {
        let Some(patch) = find_patch(snapshot, &finding_id) else {
            return ReviewMCPToolResponse::err("Error: no prepared patch artifact found. Run review_prepare_patch first.");
        };
...
fn validate_filters(args: &std::collections::HashMap<String, String>) -> Option<String> {
    ...
    if !severity.is_empty() && !matches!(severity.as_str(), "critical" | "warning" | "suggestion" | "info") {
        return Some(format!("Error: invalid severity '{}'. Use: critical, warning, suggestion, info", severity));
    }
    let status = trimmed_arg(args, "status").to_lowercase();
    if !status.is_empty() && !matches!(status.as_str(), "open" | "fix_applied" | "patch_preparing" | "patch_ready" | "patch_applying" | "patch_applied" | "patch_failed" | "pr_opened" | "merged" | "blocked" | "dismissed" | "wont_fix" | "new" | "verifying" | "verified" | "rejected_false_positive" | "inconclusive") {
        return Some(format!("Error: invalid status '{}' for code review items", status));
    }
    let origin = trimmed_arg(args, "origin");
    if !origin.is_empty() && !matches!(origin.as_str(), "reviewer" | "bugHunter" | "securityAuditor" | "audit_tool") {
        return Some(format!("Error: invalid origin '{}'. Use: reviewer, bugHunter, securityAuditor, audit_tool", origin));
    }
    let category = trimmed_arg(args, "category").to_lowercase();
    if !category.is_empty() && !matches!(category.as_str(), "correctness" | "regression" | "concurrency" | "security" | "tests" | "maintainability" | "performance" | "other") {
...
fn normalized_scope(raw: &str) -> String {
    ...
    if trimmed.is_empty() { "uncommitted".to_string() } else { trimmed }
}
