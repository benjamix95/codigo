...
pub extern "C" fn review_core_patch_start_runtime(input: *const c_char) -> *mut c_char {
    with_raw_json_input(input, |raw| {
        let request: crate::review_patch::models::ReviewPatchRuntimeStartRequest = match serde_json::from_str(raw) {
            Ok(request) => request,
            Err(err) => return encode_raw(&crate::review_patch::models::ReviewPatchRuntimeResponse::err("decode_failed", &err.to_string())),
        };
        if request.schema_version != 1 {
            return encode_raw(&crate::review_patch::models::ReviewPatchRuntimeResponse::err("unsupported_schema", "schemaVersion must be 1"));
        }
        encode_raw(&start_runtime(request))
    })
}
...
pub extern "C" fn review_core_patch_apply_runtime_result(input: *const c_char) -> *mut c_char {
    with_raw_json_input(input, |raw| {
        let request: crate::review_patch::models::ReviewPatchRuntimeResultRequest = match serde_json::from_str(raw) {
            Ok(request) => request,
            Err(err) => return encode_raw(&crate::review_patch::models::ReviewPatchRuntimeResponse::err("decode_failed", &err.to_string())),
        };
        if request.schema_version != 1 {
...
pub extern "C" fn review_core_patch_get_runtime_state(input: *const c_char) -> *mut c_char {
    with_raw_json_input(input, |raw| {
        let request: crate::review_patch::models::ReviewPatchRuntimeStateRequest = match serde_json::from_str(raw) {
            Ok(request) => request,
            Err(err) => return encode_raw(&crate::review_patch::models::ReviewPatchRuntimeResponse::err("decode_failed", &err.to_string())),
        };
        if request.schema_version != 1 {
...
pub extern "C" fn review_core_command_plan(input: *const c_char) -> *mut c_char {
    with_raw_json_input(input, |raw| {
        let request: crate::review_command::models::ReviewCommandPlanRequest = match serde_json::from_str(raw) {
            Ok(request) => request,
            Err(err) => return encode_raw(&crate::review_command::models::ReviewCommandPlanResponse::error(err.to_string())),
        };
        if request.schema_version != 1 {
            return encode_raw(&crate::review_command::models::ReviewCommandPlanResponse::error("schemaVersion must be 1"));
        }
...
pub extern "C" fn review_core_command_mutate_snapshot(input: *const c_char) -> *mut c_char {
    with_raw_json_input(input, |raw| {
        let request: crate::review_command::models::ReviewCommandMutationRequest = match serde_json::from_str(raw) {
            Ok(request) => request,
            Err(err) => return encode_raw(&crate::review_command::models::ReviewCommandMutationResponse::error(err.to_string())),
        };
        if request.schema_version != 1 {
