...
use crate::review_panel_runtime::{
    ...
    ReviewPanelChatStartRequest, ReviewPanelEventReduceRequest, ReviewPanelIntentRequest,
...
pub extern "C" fn review_core_panel_run_start(input: *const c_char) -> *mut c_char {
    decode_call(input, |request: ReviewPanelRunStartRequest| {
        start_run_runtime(request)
    })
}
...
pub extern "C" fn review_core_panel_run_finish(input: *const c_char) -> *mut c_char {
    decode_call(input, |request: ReviewPanelRunFinishRequest| {
        finish_run_runtime(request)
    })
}

#[no_mangle]
pub extern "C" fn review_core_panel_chat_start(input: *const c_char) -> *mut c_char {
    decode_call(input, |request: ReviewPanelChatStartRequest| {
        start_chat_runtime(request)
    })
}
...
pub extern "C" fn review_core_panel_chat_finish(input: *const c_char) -> *mut c_char {
    decode_call(input, |request: ReviewPanelChatFinishRequest| {
        finish_chat_runtime(request)
    })
}

#[no_mangle]
pub extern "C" fn review_core_panel_apply_intent(input: *const c_char) -> *mut c_char {
    decode_call(input, |request: ReviewPanelIntentRequest| {
        apply_intent(request)
    })
}
