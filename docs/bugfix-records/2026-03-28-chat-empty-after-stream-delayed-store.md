...
### Fix 23 — cache pipeline da blocchi persistiti; H26 con `assistantMessageId` (mar 2026)

- **Evidenza post–Fix 22:** **H36** con `pipeMarkers` fino a **77** sul turno corrente (`b8e07f0c-…`) ma **H26** con **`traceCount` 88** e `0T,3G` nello stesso run → quasi sicuramente **altro** messaggio assistente (history) o **RAM** senza entry in `pipelineTurnStateByAssistantMessageId` pur avendo **marker già salvati** nello store; niente **H39** se Rust non svuota la timeline.
- **Fix:** `hydratePipelineTurnCacheFromPersistedAssistantMessagesIfNeeded()` in coda a `refreshMessagesSnapshot`: per ogni messaggio assistente con `toolMarker` nei `blocks` persistiti e slot cache vuota, `restoreChatTurnState` + `cachePipelineTurnStateForAssistantMessage`. Log throttled **H40** (`pipeline_turn_cache_hydrated_from_persisted_blocks`, `runId`: `pipeline-cache-hydrate23`). **H26** arricchito con `assistantMessageId` / `conversationId` nel payload NDJSON per correlare col merge.
- **Non fatto:** bump della sequence del solo `primaryText` quando mancano i marker — in conflitto con `testInterleaverKeepsSinglePrimaryBeforeToolsWhenNoToolMarkers` (riepilogo prima dei tool).
