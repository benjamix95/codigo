...
- Causa probabile: configurazione del target test/bundle packaging incoerente nello scheme o nel target `SoloCodeIntegrationTests`.
...
- Strategia di fix minimo: investigare il target test per verificare product name, executable name e packaging del bundle `.xctest`.
- Verifica post-fix:
