...
final class TaskActivityStoreScopedActivitiesTests: XCTestCase {
    ...
    func testPlanRelevantRecentActivitiesScopesBeforeApplyingLimit() {
        let store = TaskActivityStore()
        let targetConversationId = UUID()
        let otherConversationId = UUID()

        store.addActivity(makeActivity(type: "plan_step_upsert", conversationId: targetConversationId))
        for _ in 0..<30 {
            store.addActivity(makeActivity(type: "plan_step_upsert", conversationId: otherConversationId))
        }
        store.flushPending()
    ...
    func testIngestCodeReviewSnapshotDefersPersistenceOffMainThread() async {
    ...
    func testPersistenceBridgeCoalescesLatestSnapshotPerSession() async {
        let recorder = SnapshotRecorder()
        let bridge = TaskActivityPersistenceBridge(
            writeCodeReviewSnapshot: recorder.record(snapshot:)
        )
