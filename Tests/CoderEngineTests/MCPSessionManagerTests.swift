...
final class MCPSessionManagerTests: XCTestCase {
    ...
    func testResourcesAndPromptsUseRustLifecycleBackend() async throws {
        guard let lifecycleBinaryPath = locateRustLifecycleBinary(named: "mcp-lifecycle-backend-rust"),
              let fakeServerBinaryPath = locateRustLifecycleBinary(named: "fake-mcp-server") else {
            throw XCTSkip("Rust lifecycle backend binaries not found in Native/target")
        }
        ...
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent("mcp-rust-lifecycle-resources-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: workspace, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: workspace) }
        ...
        let server = makeServer(
            source: "test",
            origin: "manual",
            path: "/tmp/mcp-rust-lifecycle-resources.json",
            name: "fake-mcp-resources",
            command: fakeServerBinaryPath,
            args: [bootCountFile.path]
        )
        let manager = MCPSessionManager(
        ...
        let resources = try await manager.listResources(serverId: server.id, idleTTLSeconds: 0)
        XCTAssertEqual(resources.map(\.uri), ["fake://welcome"])
        ...
        XCTAssertEqual(resource.text, "resource:fake://welcome")

        let templates = try await manager.listResourceTemplates(
            serverId: server.id,
            idleTTLSeconds: 0
        )
        XCTAssertEqual(templates.first?.uriTemplate, "fake://template/{id}")

        try await manager.subscribeResource(serverId: server.id, uri: "fake://welcome")
        await manager.unsubscribeResource(serverId: server.id, uri: "fake://welcome")

        let prompts = try await manager.listPrompts(serverId: server.id, idleTTLSeconds: 0)
        XCTAssertEqual(prompts.map(\.name), ["review_summary"])
        XCTAssertEqual(prompts.first?.arguments.first?.name, "topic")

        let prompt = try await manager.getPrompt(
            serverId: server.id,
            name: "review_summary",
