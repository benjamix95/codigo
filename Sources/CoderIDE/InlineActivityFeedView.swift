...
// MARK: - Text Shimmer Effect (shimmer confined to text glyphs)

/// A ViewModifier that adds a sweeping shimmer highlight masked to the content shape.
/// Use on Text views to create a gleam effect on the text itself, not the background.
...
extension View {
    /// Applies a text-only shimmer effect when active.
