...
final class CLIAccountsStore: ObservableObject {
    ...
    /// 1) linked global OS accounts (when credentials are already present)
    /// 2) at least one account per provider.
    ...
    private func ensureGlobalAccountsIfNeeded() {
        var didChange = false
        for provider in CLIProviderKind.allCases {
    ...
    private func shouldAddGlobalAccount(provider: CLIProviderKind, profilePath: String) -> Bool {
    ...
    private func hasAccount(provider: CLIProviderKind, profilePath: String) -> Bool {
    ...
    private func globalProfilePath(for provider: CLIProviderKind) -> String? {
        switch provider {
        case .codex:
